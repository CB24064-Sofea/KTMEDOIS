    ...
    <div class="details-grid">
        ...
        <div>
            <div class="decision-card">
                ...
                <button style="display:block;width:100%;background-color:#d97706;color:#fff;border:none;padding:14px;border-radius:8px;font-weight:700;font-size:14px;cursor:pointer;margin-bottom:10px;" onclick="openModal('UnderReview')">&#8635; Request Additional Info</button>
...
function openModal(action) {
    ...
    document.getElementById('modalIcon').className = 'modal-icon ' + (isApprove ? 'icon-approve' : isRequest ? '' : 'icon-reject');
    document.getElementById('modalIcon').textContent = isApprove ? '✓' : isRequest ? '↻' : '✗';
    document.getElementById('modalIcon').style.background = isApprove ? '#16a34a' : isRequest ? '#d97706' : '#dc2626';
    document.getElementById('modalTitle').textContent = isApprove ? 'Confirm Approval' : isRequest ? 'Request Additional Info' : 'Confirm Rejection';
    document.getElementById('modalSub').textContent   = isApprove
        ? 'Approve this invoice and route to Finance Review queue?'
        : isRequest
        ? 'Request additional information? Invoice will be set to Under Review and vendor will be notified.'
