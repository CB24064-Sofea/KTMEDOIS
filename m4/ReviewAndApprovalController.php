...
class ReviewAndApprovalController {
    ...
    // ── fetchPaymentQueue() — Finance Officer "Process payments" ─────────────
    // Returns invoices that are fully Approved and awaiting/undergoing payment.
    public function fetchPaymentQueue() {
        return $this->invoiceModel->fetchPaymentQueue();
    }

    // ── processPayment() — SDD_CLS_405 ───────────────────────────────────────
    // Finance Officer marks a fully-approved invoice's payment_status as Paid.
    // Separate from updateStatus() because payment processing happens AFTER
    // the invoice_status has already reached its final 'Approved' state.
    public function processPayment($invoiceId) {
        ...
        if ($invoice['invoice_status'] !== 'Approved') {
            return ['success' => false, 'message' => 'Only fully approved invoices can be marked as paid.'];
        }
        if ($invoice['payment_status'] === 'Paid') {
            return ['success' => false, 'message' => 'This invoice has already been marked as paid.'];
        }
        ...
        // logActivity() → AuditController.writeLogEntry()
        $this->logActivity('PaymentProcessed', $invoiceId);

        // Notify vendor payment has been completed
        $inv = $this->invoiceModel->getInvoiceWithCustomer($invoiceId);
        if ($inv) {
            $this->invoiceModel->insertNotification(
                $inv['customer_ID'],
                'Payment Completed',
                "Invoice {$inv['invoice_num']} has been paid in full. Payment disbursement is complete."
            );
        }
